...
class caogaoModel extends baseModel{
	...
	function caogao_all(){
		...
		return $this -> db -> sql("select * from article inner join article_type on article.t_id = article_type.t_id where article.caogao = 0 order by article.`top`,article.`sortop`");
	...
	function article_seach() {
		...
		$re = $this -> db -> sql('select * from article inner join article_type on article.t_id=article_type.t_id where article.caogao=0 article.t_id = '.$_GET['id']);
		if($re == false){
	...
	function caogao_add() {
		...
		$id = $_POST['id'];
		foreach($id as $k => $v){
			$re = $this -> db -> Up("article",array('id'=>$v),array('caogao'=>'1'));
		}
		return $re;
	...
	function article_sou() {
		...
		$sou = $_POST['keyword'];
		$re = $this -> db ->sql("select * from article left join article_type on article.t_id=article_type.t_id where article.caogao=0 and article.title like '%".$sou."%'");
		return $re;
	...
	function article_all() {
		...
		return $this -> db -> sql("select * from article where caogao=0 order by `top`,`sortop` ");
	...
	function author_seach() {
		...
		$re = $this -> db -> sql("select * from article where author = '".$_GET['id']."' and caogao=0 ");
		if($re == false){
