...
<div id="post">
    <div>
        <input value='{$re.title}' type="text" maxlength="200" name="title" id="title"/>
    </div>
    <div>
<span class="field">
        <input type="hidden" value="{$re.id}"  name="id">
            <textarea id="editor1" onblur='editor1()' name="sit_content"><{$re.content}></textarea>
            &nbsp;&nbsp;&nbsp;&nbsp;<sp id='sp_editor1'></sp>
        </span>
    ...
    <div style="margin:10px 0px 5px 0px;">
        ...
        {foreach from = $arr item = v}
            ...
            <option value="{$v['t_id']}" {if $re.t_id eq $v.t_id}selected="selected"{/if}>{$v['t_name']}</option>
        ...
        发布于：<input  type="date" style="width:139px;" name="postdate" id="postdate" value="{$re.datetime}"/>
        <div id="tagbox">
        {if $tag}
        {foreach $tag as $k=>$v}
