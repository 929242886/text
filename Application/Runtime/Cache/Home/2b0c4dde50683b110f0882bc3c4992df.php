...
<div id="body">
    ...
    <dl id="user_sideBarmenu" class="user_sideBarmenu">
     	        ...
     	        <dd><a href="/index.php/Home/Usercenter/index">我的简历</a></dd>
                <dd><a class="hover" href="/index.php/Home/Usercenter/invite">收到邀请</a></dd>
        ...
        <dd>
            <table class="hovertable">
                <tr>
                    ...
                    <th>地点</th>
                    <th>公司简介</th>
                    <th width="30px">待遇福利</th>
                    <th width="30px">招聘岗位</th>
                    <th>薪资</th>
                    <th>面试时间</th>
                    <th>是否显示面试时间</th>
                    <th>是否接受邀请</th>
                ...
                <tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
                    ...
                    <td><?php echo $v['e_area']; ?></td>
                    <td><?php echo $v['e_description']; ?></td>
                    <td><?php echo $v['l_benefit']; ?></td>
                    <td><?php echo $v['l_job']; ?></td>
                    <td><?php echo $v['l_salary']; ?></td>
                    <td><?php echo $v['l_time']; ?></td>
                    <td>
                        <a href="javascript:void(0)" onclick="times(this,<?php echo $v['id']?>)"><?php if ($v['is_show']=='0') { ?>是<?php }else{ ?>否<?php } ?></a>
                    </td>
                    <td>
                        <a href="javascript:void(0)" onclick="invites(this,<?php echo $v['id']; ?>)" ><?php if ($v['is_accept']=='0') { ?>是<?php }else{ ?>否<?php } ?></a>
                    </td>
                </tr>
                <?php endforeach ?>
            </table>          	
...
<script type="text/javascript">
    ...
    function times(th,id){
        var ids = $(th).html();
        if(ids == '是'){
            if(confirm('您确定取消面试时间吗？') == false){
                return false;
            }
        }else{
            if(confirm('您确定显示面试时间吗？') == false){
                return false;
            }
        }
        $.get("/index.php/Home/Usercenter/mianshi",{'id':id},function(obj){
            if(obj == 1){
                if(ids == '是'){
                    $(th).html('否');
                }else{
    ...
    //是否接受面试邀请
    function invites(th,id){
        var ids = $(th).html();
        if(ids == '是'){
            if(confirm('您确定拒绝面试邀请吗？') == false){
                return false;
            }
        }else{
            if(confirm('您确定接受面试邀请吗？') == false){
                return false;
            }
        }
        $.get("/index.php/Home/Usercenter/inv",{'id':id},function(obj){
            if(obj == 1){
                if(ids == '是'){
                    $(th).html('否');
                }else{
...
<!------------------------------------- end -----------------------------------------> <script src="/Public/styles/js/setting.js"></script>
			<div class="clear"></div>
